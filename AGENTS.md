...
| What time it is **at the club** — the installation's timezone, deriving a civil date or time from an instant, or anything that reads `TZ` / `NEXT_PUBLIC_TZ` / `APP_TIME_ZONE` | `INV-CONFIG-002` → [`product-configuration.md`](docs/invariants/product-configuration.md), plus `INV-DATE` → [`booking-dates-and-capacity.md`](docs/invariants/booking-dates-and-capacity.md) | [`guides/club-time.md`](docs/guides/club-time.md). The club timezone is ONE persisted IANA identifier read through `getClubTimeZone()`; the environment only seeds it, the browser never decides it, and the fix for a zone bug is never to pin the process zone harder. `APP_TIME_ZONE` is transitional and retired by CT-6 (#2991) |
