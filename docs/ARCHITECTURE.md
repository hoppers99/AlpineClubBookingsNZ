...
| `hosting-coverage-reevaluation` | Every 3 hours | Drain the bounded same-owner hosting-coverage queue (#2576). Every path that can take adult-member cover away from another booking on the same account records the owner, lodge and exact nights to re-examine inside its own transaction, and drains that inline right after committing; this sweep is the BACKSTOP and the authority on completion, so a process that died mid-drain, a redeployment or a transient email failure cannot leave a confirmed booking uncovered with nobody told. Per item it re-reads the committed facts for that owner/lodge/nights only — never a lodge-wide sweep — then opens, updates or resolves one urgent compliance incident per booking and emails the owner once per genuine change of state. Idempotent throughout: an empty queue costs one indexed read, and re-running the same unchanged problem writes nothing and sends nothing. It never changes a booking's status |
