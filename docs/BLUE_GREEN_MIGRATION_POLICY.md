...
- **`previous_expand_release` had nothing truthful to name.** The runtime half this contracts against is PR #2565, which shipped **no migration at all** — it removed every reader and writer and added an `@ignore`, which changes no database state. So the field names an *adjacent* migration in the same release (the first windowed row above) and the `lock_impact_plan` carries the real sequencing fact, exactly as the #2130 contract row `20260721130000` did for its own runtime-prep release. `previous_expand_release` is documentation the validator only checks for non-emptiness; when it cannot be true, say what is true in the plan column rather than inventing a folder.
...
- **The rollback restores the SHAPE but not the VALUES.** PostgreSQL cannot un-drop a column, so `rollback.sql` recreates it and refills every row with a single **safe compatibility value**. Where a `windowed` rollback cannot reconstruct history, the row and the script must both say which value was chosen, why it is safe, and what choosing it costs — here `'MEMBER'`, because it is the column's own default (so it is the *actual* value of every row written since the runtime half) and the least-privileged label that ever existed, at the cost of one fail-closed convenience path on a rolled-back release. Reconstructing the privileged label by heuristic was considered and rejected: guessing in the granting direction is the one mistake a rollback script must not make. The paired practice is on the operator side — the runbook's pre-migration checks take a per-row dump so an exact restore stays available to anyone who wants it.
