...
| **Per-lodge capacity** | `hashtextextended(<lodgeId>, 0)` | `acquireLodgeCapacityLock(tx, lodgeId)` (`lodge-capacity-lock.ts`, re-exported by `capacity.ts`) | 1 | Capacity claims/checks and bed-allocation mutations for one lodge; booking admission versus a lodge's ACTIVATION or deactivation — both directions of `PATCH /api/admin/lodges/[id]` take this key, and since #221 activation is a first-class operator action (the per-lodge setup flow's finish step opens a lodge that was created closed), not merely the undo of a deactivation; hut-leader overlap and optional bed-hold writes; roster eligibility snapshots; and direct or config-transfer chore-template changes, which serialize active-template validation for that lodge. |
