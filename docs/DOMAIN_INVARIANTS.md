...
| `INV-CONFIG-006` | Making the public site visible consumes the canonical role and is refused while it is UNKNOWN; both writers of `ClubTheme.completedAt` ask the one gate, a declared role of either kind publishes freely, and the whole request is refused rather than the completion half dropped |
