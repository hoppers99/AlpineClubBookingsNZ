...
| `INV-MOD-027` | An officer may price a guest who is on the club's non-member rate at its member rate as a partner-lodge member; eligibility follows the RATE rather than `isMember`, excludes a member repriced by an unpaid subscription, changes the rate only, and reprices exactly the guests whose tick changed |
