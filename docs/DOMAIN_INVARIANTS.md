...
| `INV-CONFIG-006` | Making the public site visible consumes THREE registry facts — the canonical role, the runtime environment contract, and the auth secret's strength — and is refused, in that deterministic order, while any one is not complete; both request-path writers of `ClubTheme.completedAt` ask the one gate (reusing the wizard's own check functions and remedy copy) and a census keeps that list complete, the seed and the E2E harness write the column directly and are deliberately out of scope, a declared role of either kind publishes freely, and the whole request is refused rather than the completion half dropped |
