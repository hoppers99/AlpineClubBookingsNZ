...
| `e2e/setup-wizard-rail.spec.ts` | Setup wizard shell and rail (#220, epic #213; Medium) | The guided setup journey, driven by clicking. The launcher on `/admin/setup` opens `/admin/setup/wizard`; every applicable step has a rail row under a category heading; the percentage on screen is the traversal's own (D7) and the wizard opens on the resume step with its row in view. A step past the navigation frontier is **not a control at all** — the row is a `div` with no handler, so clicking where it sits changes nothing (D2 enforced at the control rather than by a redirect); that target is derived as the first unreachable step AFTER the resume point, and the test skips with a stated reason on a seed that leaves none, rather than asserting on a row behind the frontier or throwing on `undefined`. **Skip for now** buys passage — the next row becomes reachable — while the skipped step stays on the rail stated as outstanding (D4), and **Reopen** takes the passage back. **Mark this step done** moves the row to Done and raises the percentage — which on a seeded stack starts at 0% however many checks pass, because D14 (#237) counts confirmations rather than passing checks, and rows whose check passes unconfirmed read **Default in place**. That state blocks the walk exactly as an untouched step does (D15), so the first unreachable row after the resume point is derivable rather than accidental. The last test is mockup 2: `xeroIntegration` toggled through `PUT /api/admin/modules` adds and removes its two rail rows with no navigation, driven with two real pages and `bringToFront` so the redraw is triggered by the browser's own `visibilitychange` rather than by a synthetic event. Resets setup progress before and after (#2302) |
| `e2e/setup-wizard-launch.spec.ts` | Setup wizard resume + launch panel (#220, epic #213; Medium) | The two states only a running stack shows. **Resume:** two steps completed through the progress API, after which reopening the wizard lands on the resume point rather than the beginning, with its rail row in view and the finished steps still reachable in both directions. **Launch (D9):** deferring everything still outstanding unlocks **Ready to open** at the foot of the rail; the panel states what was skipped rather than hiding it (mockup 6), and its environment-role section (C9, #224) names the role from upstream's ENV-SAFETY resolution and instructs — naming `.env` — with no control at all, because the role is consume-only by design and never switched on from the wizard. Reopening one deferred step locks the panel again. **The environment split (D17, #246)** adds two: *the environment facts are reported on their own panel, not walked* reads the traversal back from the API, asserts every fact is absent from `steps` and present in `applicableStepIds` (the readiness cards read that one), then opens the rail's **About this server** row and requires a visible panel row for each fact — driven from the product's own fact list rather than a hard-coded five, so a reclassification moves the test with it; and *the progress route refuses a transition on an environment fact* PATCHes `complete` for the first fact and requires **422**, which is the server half of "nobody may confirm a deployment fact". Both resume assertions read the resume point back from the product rather than assuming a registry position. The ORIGINAL reason for that — that several readiness checks pass on their own in a seeded stack, so a fixed index would pass vacuously — was inverted by D14 (#237): a passing check no longer moves the resume point at all, which is now always the first step nobody has CONFIRMED, so a fresh stack resumes at the very beginning. The practice stays right for a different reason, and a stronger one: the resume point moves with the applicable set, which module flags change, and with whatever the spec's own earlier steps confirmed. Resets setup progress before and after (#2302) |
...
2. Drops and recreates the `public` schema, then runs `prisma migrate deploy`,
   ...
   database before the destructive demo seed guard runs. It then writes the
   persisted club identity the product seed no longer writes on its own
   (`e2e/setup/seed-club-identity.ts` — since #237 the seed writes one only from
   a real committed `config/club.json`, which is gitignored, so in CI it
   correctly writes nothing and every setup surface would otherwise report the
   stack blocked), and enables the modules the E2E journeys need
