...
| `e2e/setup-wizard-launch.spec.ts` | Setup wizard resume + launch panel (#220, epic #213; Medium) | The two states only a running stack shows. **Resume:** two steps completed through the progress API, after which reopening the wizard lands on the resume point rather than the beginning, with its rail row in view and the finished steps still reachable in both directions. **Launch (D9):** deferring everything still outstanding unlocks **Ready to open** at the foot of the rail; the panel states what was skipped rather than hiding it (mockup 6), and its environment-role section instructs — naming `.env` — with no control at all, because the role is consume-only until upstream ENV-SAFETY lands (#224). Reopening one deferred step locks the panel again. Both resume assertions read the resume point back from the product rather than assuming a registry position, because several readiness checks pass on their own in a seeded stack and a fixed index would pass vacuously. Resets setup progress before and after (#2302) |
