...
| `e2e/asset-url-404.spec.ts` | No matrix row — regression guard for #2404 (High) | The CSP nonce on URL shapes `src/proxy.ts` skips, which only a running server can show. Anonymous throughout — these are the addresses scanners and stale browser tabs ask for. A MISSING asset (`/foo.png`, `/favicon.ico`, `/logo.png`, `/wp-content/uploads/x.jpg`, `/branding/definitely-missing.png`, `/_next/static/chunks/nope.js`) must answer **404 with an empty body, no `content-type`**, its own `default-src 'none'` policy and the app's security headers — never the ~29KB HTML 404 page, which shipped inline scripts with no nonce and no CSP header at all. A REAL asset must still be served: `/branding/favicon.example.ico` and a `/_next/static/chunks/*.js` URL taken from a live page render both 200 with their bytes, which is the assertion that catches an `afterFiles` -> `beforeFiles` slip turning every image in the app into a 404. An ADMIN-UPLOADED image must also still be served: the spec uploads a 1x1 PNG through `/api/admin/image-manager/upload` (the uploads directory is a container volume the seeds never populate), fetches it back **anonymously** at `/api/images/uploaded/<file>` and asserts 200 with the same bytes and `image/png` — the guard against the `/api` asset rule swallowing every uploaded picture in the app — while a missing one still gets that route's own JSON 404. `/_next/staticfoo`, `/_next/imagemap`, `/_next/image/x`, `/apiary` and `/api-docs` must carry a nonced policy (the bare-prefix exclusions anchored in #2404 and #2420). `/definitely-missing` must still render the club's own 404 screen, fully nonced — nothing here may blank a human-plausible mistyped address. `/api/does-not-exist.png`, `/api/chores/zzz.svg` and `/API/x.png` must still answer the JSON 404, because routing an asset-shaped `/api` URL elsewhere would reopen #2405's module-state oracle |
