...
| `src/lib/email-templates.ts` (deleted) | — | Split (#2689) into 22 cohesive modules under `src/lib/email-templates/`, one per message family, with **no compatibility barrel** — callers import the family module directly. The families mirror the sender modules in `src/lib/email/` (`account`, `booking`, `booking-requests`, `chores`, `family`, `family-booking`, `groups`, `member-guest`, `membership`, `waitlist`, `admin-booking`, `admin-finance`, `admin-membership`, `admin-ops`), plus `communications` and `refunds` for the two families sent outside `src/lib/email/` — `refunds` from the admin refund-request route, `communications` from the admin communications route and the `server-only` lib module `src/lib/notices-email.ts` — four size/cohesion sub-modules (`booking-reminders`, `booking-exceptions`, `admin-xero-reports`, and `booking-money` — which is cross-family, imported by `admin-booking` as well as `booking`, and is domain money logic rather than rendering: it holds no HTML and is cited as the authoritative netting contract by `xero-credit-sync-checker.ts`. It sits here only because both of its consumers are email paths; moving it to a domain module is a reasonable follow-up and was out of scope for a behaviour-preserving split), the shared shell `layout` (which is not a leaf — it reads the club theme and the email settings), and the leaf `escape`, which imports nothing. Largest module 581 LOC, all inside the 700 budget. Rendered output is pinned byte-for-byte by `src/lib/__tests__/email-render-equivalence.test.ts` — 217 complete bodies, and the module list is read from the directory so a new module cannot arrive uncovered. `REGISTRY_KEYS_WITHOUT_A_TEMPLATE_FUNCTION` is empty: the two keys that used to build their HTML at a send site (`website-contact`, `admin-email-failure`) were moved into `admin-ops` verbatim so the gate can see them. It was proved able to fail before the move (three deliberate mutations, two of them byte-length-neutral) and stayed green across it; review added six more mutations, including reducing `escapeHtml` to `return str` and dropping only its `&` and `'` cases. |
| `src/lib/contextual-help.ts` (deleted) | — | Split (#2689) into 16 modules under `src/lib/contextual-help/`. `index.ts` **is** the registry (path matching, longest-prefix resolution, fallbacks, question attachment) rather than a barrel, and keeps the same three exported accessors; entry content sits in one module per **admin sidebar section** (`admin/*.ts`, matching `navSections` in `admin-sidebar.tsx`) — plus one `appearance-and-website` module split off Setup & Configuration, because `/admin/appearance` is an item in that section rather than a section of its own and folding its seven pages back would take that module to ~810 lines, over budget — with `finance.ts`, `questions-*.ts`, `fallbacks.ts`, and the two leaves `types.ts` and `booking-status-glossary.ts`. Content stayed TypeScript by owner decision — the typed shape is the schema check. Largest module 604 LOC. Proved value-for-value FOR EVERY RESOLVED PATH: a JSON dump keyed by path — every registered path in both scopes, both fallbacks, nested resolution and `normalisePath` — is byte-identical before and after (106,917 bytes, same sha256). Being keyed by path, it does not observe two things: the order `getContextualHelpPaths` returns (which changed, and which nothing outside tests reads), and the one duplicate `/admin/notifications` entry that longest-prefix resolution has always shadowed, so 68 of the 69 entries are covered. |
