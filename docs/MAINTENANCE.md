...
| `src/lib/email-templates.ts` | 0 | Split (#2689) into 22 cohesive modules under `src/lib/email-templates/`, one per message family, with **no compatibility barrel** — callers import the family module directly. The families mirror the sender modules in `src/lib/email/` (`account`, `booking`, `booking-requests`, `chores`, `family`, `family-booking`, `groups`, `member-guest`, `membership`, `waitlist`, `admin-booking`, `admin-finance`, `admin-membership`, `admin-ops`), plus `communications` and `refunds` for the two families sent from routes rather than a sender, four size/cohesion sub-modules (`booking-reminders`, `booking-exceptions`, `booking-money`, `admin-xero-reports`), and the two shared leaves `layout` and `escape`. Largest module 593 LOC, all inside the 700 budget. Rendered output is pinned byte-for-byte by `src/lib/__tests__/email-render-equivalence.test.ts`, which was proved able to fail before the move (three deliberate mutations, two of them byte-length-neutral) and stayed green across it. |
| `src/lib/contextual-help.ts` | 0 | Split (#2689) into 16 modules under `src/lib/contextual-help/`. `index.ts` **is** the registry (path matching, longest-prefix resolution, fallbacks, question attachment) rather than a barrel, and keeps the same three exported accessors; entry content sits in one module per **admin sidebar section** (`admin/*.ts`, matching `navSections` in `admin-sidebar.tsx`), with `finance.ts`, `questions-*.ts`, `fallbacks.ts`, and the two leaves `types.ts` and `booking-status-glossary.ts`. Content stayed TypeScript by owner decision — the typed shape is the schema check. Largest module 605 LOC. Proved value-for-value: a JSON dump of every resolved path, both fallbacks, nested resolution and `normalisePath` is byte-identical before and after (106,917 bytes, same sha256). |
