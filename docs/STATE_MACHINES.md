...
parent link requested -> parent is active + not archived (ANY age tier) -> ancestors(parent) + 1 + descendants(child) <= 3 links -> linked | 422 (parent inactive/archived) | 422 (four-generation cap) | 422 (would close a family loop)
...
can genuinely be a parent, so recording the relationship is age-blind while every
responsibility function — the contact of record for a dependant's mail, details
confirmation, booking delegation — stays gated on an active adult, none of which
consults the parent link. A dependant of a non-adult parent therefore inherits
email from the nearest adult ancestor, and the admin member page names that adult
before the dependant is added.
