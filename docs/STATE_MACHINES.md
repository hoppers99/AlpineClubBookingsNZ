...
subscription invoice when the approval is itself about to credit it IN FULL —
mirroring `queueApprovedMembershipCancellationXeroOperations`, which would
otherwise deadlock the ordinary unpaid-subscription cancellation. "In full" is
decided by `loadMembershipCancellationSubscriptionCreditPlansByMemberId`
(#2400), the same module the credit note consults, so the exclusion set is
exactly the set the approval clears: an invoice that will NOT be credited —
because it still covers other members who have not been cancelled — blocks like
any other, or the archive would take a Xero contact with a live balance out of
circulation. If Xero cannot be
...
See [`CANCELLATIONS.md`](CANCELLATIONS.md#shared-family-invoices).
