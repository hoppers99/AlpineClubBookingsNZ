...
- `buildSyncHealthDashboard` in `finance-dashboard-page.ts` maps sections onto the existing KPI-card/status-panel shapes; status-panel items may carry `href`/`linkLabel` (Xero admin console `/admin/xero`, report mappings `/finance#finance-report-mappings`, the dashboard's own collapsed editor since #223).
