...
| After switching **Rules for** to a lodge, the section says "Could not load the policy for &lt;lodge&gt;" and shows no editor | That lodge's policy could not be fetched, so what is stored for it is unknown | Switch scope again or reload the page. Nothing is shown deliberately: the policy still on screen belongs to a different scope, and editing or removing it from here would change the wrong thing |
