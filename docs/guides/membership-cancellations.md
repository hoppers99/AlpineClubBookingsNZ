...
1. In the **Cancellation Review Queue**, open a request and its participants. A
   ...
   If your admin role can **view** membership but not edit it, you see a short
   blue note on those rows instead: *Approval checks were not run for this
   member.* Asking Xero what a contact owes uses one of the club's metered API
   calls, so the queue only spends one for an admin who could act on the answer.
   Read that note literally — it does not mean the member owes nothing, it means
   nobody asked. An admin who can approve sees the real answer on this same page.
...
| A row says "Approval checks were not run for this member" | Your admin role has membership view but not edit. Asking Xero what a contact owes costs one of the club's metered API calls, so the queue only spends one for an admin who could act on the answer — the same reason the Approve and Reject buttons are disabled for you | Nothing is wrong with the request. Read the note literally: it does **not** say the member owes nothing, it says nobody asked. Ask an admin with membership edit access — they see the real answer, and the amber notice if there is one, on this same page. Approval itself always runs the check live, so nothing can slip through unchecked |
