...
Prefix defined in this file: **`INV-GUEST`** â€” when a member added as somebody
...
Index: [`docs/DOMAIN_INVARIANTS.md`](../DOMAIN_INVARIANTS.md) â€” every `INV-*` ID
...
verbatim move from the source document and must not be reworded in place** â€”
...
columns on `BookingGuest` â€” `consentStatus`, `consentRequestedAt`,
`consentRespondedAt`, `consentRespondedByMemberId`, `consentExpiresAt` â€” not in
a side table.
...
resolves and the row carries a real consent state; with it disabled â€” the shipped
default (D-2) â€” a cross-family add is refused with the byte-for-byte pre-feature
...
- **An approved stay may be extended without asking again** (owner decision
  ...
  holding the new nights without being asked. Their only exit is self-removal â€”
...
- **A member who never consented can be refused the ability to come off the
  ...
  election. They are TRAPPED â€” a plain-English 400, and the row survives as
  *blocked* on the admin exception list â€” when a captured payment's cancellation
...
- **`NULL` is not `CONFIRMED`.** A null `consentStatus` means *no consent was
  ever needed* â€” a family-scope add (D-6) or a row written before the feature
...
- **A refusal names who refused.** `DECLINED` requires a non-null
  `consentRespondedByMemberId`. Declining is an attributed act â€” MG4's audit
  reads that column to say who turned the add down â€” so an unattributed decline
...
- **Nobody answers for a member who can sign in** (owner decision D-5/D-10). The
  ...
  the same booking can answer for each other â€” including declining, which
  ...
  may not answer for them either â€” an unanswerable request emailed to a household
...
- **A delegate's answer is told to the member it was given for.** A member
  ...
  else's decision about them, so the member â€” and the other adults who were sent
  the same request â€” receive the `member-guest-consent-answered` email naming who
...
- **A `PENDING` row holds the bed** (D-4) until `consentExpiresAt`, which is set
  from `MemberGuestSettings.pendingHoldExpiryDays` (default 7, bounds 1â€“60). A
...
- **A merged-away member's guest rows keep their consent.** `BookingGuest.member`
  ...
  re-points A's guest rows â€” consent columns included â€” onto B.
  ...
  survivor, i.e. in `LAST_GUEST` â€” so a later decline or lapse on that row lands
...
- **A pending guest is not operationally present** (owner decision D-12). Only
  ...
  double-bed candidate sweep, and â€” because these are member-facing content, not
  just screens â€” the pre-arrival and check-in reminder emails. The single shared
...
- **A pending guest DOES hold a bed and a person-night** (owner decision D-4), and
  ...
  gain a consent filter â€” a pending guest who did not hold their person-night
...
- **Every path that can place a member on a booking now records who did it and
  ...
  body named â€” the capacity hold's booking create, the approval-time guest swap,
  and the approval that runs with no hold behind it â€” and all three write
...
- **A held booking's guest swap can substitute one person for another in place.**
  ...
  the person on it changes â€” a stale `ADMIN_ASSIGNED` would vouch for somebody
...
- **"Always notify" beats the per-action tick and the member's preferences, and
  ...
  notify choice â€” being asked, being told you are on a booking, and being told
...
- **The officer's member picker gates its NAME mode on `membership:view`, not on
  ...
  what keeps #1376 true â€” a Booking Officer whose role carries no membership
