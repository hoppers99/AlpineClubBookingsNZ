...
- **Reclassifying an audit row's `category` in code changes only the rows written
  ...
  **Enforcement is partial, and the boundary is stated rather than implied.** The
  ...
  than having none: the audit-writer census pins only 127 of its 453 write sites
  ...
  something edit a 400-line literal — so for the other 326 there is no baseline
