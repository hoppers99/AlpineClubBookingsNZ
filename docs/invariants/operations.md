...
- **Reclassifying an audit row's `category` in code changes only the rows written
  ...
  **Enforcement is partial, and the boundary is stated rather than implied.** The
  ...
  than having none: the audit-writer census pins only 127 of its 456 write sites
  ...
  something edit a 400-line literal — so for the other 329 there is no baseline
