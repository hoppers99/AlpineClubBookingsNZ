...
- **Reclassifying an audit row's `category` in code changes only the rows written
  ...
  **Enforcement is partial, and the boundary is stated rather than implied.** The
  ...
  `LODGE_GATED_ADMIN_CATEGORIES_2765`, counted rather than added up —
  ...
  **The same pull request also states the MEASURED before/after audience, per
  ...
  `AUDIT_CORRELATION_DOMAIN_AREAS` for which of the five AI Diagnostics
  ...
  it; and `classifyAuditRetention` on the site's OWN action, since that function
  ...
  reached the intended reader was member-visible (`INV-PRIV-013`).
