...
- Lobby Display: a new flagged-off module for per-lodge lobby screens with
  ...
  phone number is shown. Off by default; nothing changes until enabled.
...
- Multi-lodge operation is core: the `multiLodge` module flag is removed,
  lodge routes are un-gated, the admin home becomes a lodge hub, and legacy
  single-lodge columns are retired. Single-lodge clubs see no operational
  change.
...
- The one contract migration, `20260714140000_drop_committee_member`, drops
  ...
  existed, so the drop loses no data and the required expand release is fully
  drained. The old colour's admin committee CRUD routes still read the table,
  so between migrate and cutover those admin-only routes error with
  relation-does-not-exist; public committee and contact surfaces read from
  assignments on both colours and are unaffected. Deploy with old-colour
  admin traffic idle or drained, cut over promptly, and use
...
- Two expand migrations carry window-bounded old-colour caveats documented in
  ...
  entrance-fee Xero item-code mapping, so an entrance-fee invoice minted by
  the draining old colour falls back to an account-coded line (a
  categorisation nuance, never a wrong amount); and
  `20260716140000_xero_member_grouping` converges grouping rules, so a
  membership-type rule save on the draining old colour should be avoided.
  Deploy both in a quiet window and cut over promptly.
...
- The new fee model backfills day-one amounts from the existing configuration
  and retains the legacy `SeasonRate` boolean keys, `EntranceFee` table, and
  Xero mapping amounts, so both colours price identically during cutover.
