...
test("existing-chip pointer and keyboard drops preserve dates, while keyboard cancel is silent", async () => {
  const ken = DEMO_BOOKING_WINDOWS.kenReview;
  ...
  // Leave the serial fixture on its seeded bed for the next scenario and for
  // any later spec that inspects the demo stack.
  const restoreAfterKeyboard = await adminContext.request.patch(
    "/api/admin/bed-allocation/allocations",
    {
      data: {
        allocationIds: persisted.map((allocation) => allocation.id),
  ...
  expect(
    restoreAfterKeyboard.ok(),
    `restore after keyboard drop (${restoreAfterKeyboard.status()})`,
  ).toBeTruthy();
  await page.close();
});
