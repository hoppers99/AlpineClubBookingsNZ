...
/*
  ...
  WHY THIS SPEC IS DELIBERATELY API-HEAVY, with only the button checked in the
  ...
  The one thing that genuinely IS visual — does the surface disappear cleanly
  when the club has the module off — is checked in the browser, because "the
  button is absent from the page" is the whole claim.
  ...
  MECHANICS:
    ...
    - NO BOOKING IS CREATED. The finder never touches a booking, so this spec
      needs no stay window and can never collide with another spec's nights or
      leave a person-night behind for its own retry to trip over.
...
test.describe("module OFF — the surface does not exist", () => {
  test.beforeAll(async () => {
    previousModules = await overrideModules(adminRequest, { memberGuests: false });
  });

  test("both find routes 404 and the wizard button is absent from the page", async () => {
...
test.describe("module ON, open search OFF — the default every club gets", () => {
  ...
  test("the wizard offers the button and opens the find panel inline", async () => {
    const page = await wandaContext.newPage();
    await page.goto("/book");
...
test.describe("open search ON — the browsable membership list a club opts into", () => {
  ...
  test("the wizard's one box now offers either a name or an address", async () => {
    const page = await wandaContext.newPage();
    await page.goto("/book");
