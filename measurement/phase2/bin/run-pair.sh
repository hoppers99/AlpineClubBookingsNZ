...
quiet_check() {
  ...
  node - "$dir" "$QUIET_CPU_LIMIT_PERCENT" <<'NODE'
...
for index in 0 1; do
  ...
  node measurement/phase2/bin/verify-completed-run.mjs "$PAIR_ROOT/$side" > "$PAIR_ROOT/pair-evidence/$side-completion-verified.json"
  SIDE_RECORDS+=("$sequence|$side|$restore_started|$side_started|$side_ended|$gap|$before_fingerprint|$after_fingerprint")
...
node - "$PAIR_ROOT/pair.json" <<'NODE'
const fs = require("node:fs");
const parse = (raw) => {
  const [sequence,side,restore_started_at,started_at,ended_at,gap_from_previous_seconds,database_fingerprint_before,database_fingerprint_after] = raw.split("|");
  return {sequence:Number(sequence),side,restore_started_at,started_at,ended_at,gap_from_previous_seconds:Number(gap_from_previous_seconds),database_fingerprint_before,database_fingerprint_after};
};
const pair = {
  schema_version: 2,
...
exec 1>&3 2>&4
wait "$PAIR_TEE_PID"
node measurement/phase2/bin/finalize-run.mjs --dir "$PAIR_ROOT" --side pair --pair-id "$PAIR_ID" > "$PAIR_ROOT.finalization.json"
