...
start_sampler() {
  local segment="$1"
  ( while true; do
      printf '%s,' "$(date -u +%Y-%m-%dT%H:%M:%S.%NZ)"
      docker stats --no-stream --format '{{.Name}},{{.CPUPerc}},{{.MemUsage}},{{.PIDs}},{{.BlockIO}},{{.NetIO}}' | grep '^tacbookings-measure-' || true
      sleep 1
...
segment_end() {
  ...
  docker logs --since "$(cat "$OUT/segments/$segment-started-at.txt")" "$APP" > "$OUT/segments/$segment-app.log" 2>&1 || true
...
find measurement/phase2 measurement/stack -type f ! -path '*/results/*' -print0 \
  | sort -z | xargs -0 sha256sum > "$OUT/env/harness-files.sha256"
...
for route in "${ROUTES[@]}"; do
  ...
  cpu0="$(cpu_usec)"
  : > "$OUT/warm/$safe.csv"
  for _ in $(seq 1 "$RUNS"); do ttfb_line "$BASE$route" >> "$OUT/warm/$safe.csv"; done
  cpu1="$(cpu_usec)"
  segment_end "$segment"
...
if [ "$SIDE" = current ]; then expected_revalidation='STALE|MISS'; else expected_revalidation=ABSENT; fi
node measurement/phase2/bin/verify-http-proof.mjs --manifest "$CORRECTNESS_MANIFEST" --side "$SIDE" \
...
echo "/about,1,$cpu0,$cpu1,$((cpu1-cpu0))" > "$OUT/revalidation/first-cpu-usec.csv"
for _ in $(seq 1 "$WARMUP"); do ttfb_line "$BASE/about" >/dev/null; done
