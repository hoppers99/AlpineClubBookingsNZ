...
if (import.meta.filename === process.argv[1]) {
  const arg = (name) => { const index = process.argv.indexOf(`--${name}`); if (index < 0 || !process.argv[index + 1]) fail(`--${name} is required`); return resolve(process.argv[index + 1]); };
  const harnessManifestPath = arg("harness-manifest");
  const current = verifyCorrectnessCompletion(arg("current-completion"));
  const baseline = verifyCorrectnessCompletion(arg("baseline-completion"));
  const out = arg("out");
