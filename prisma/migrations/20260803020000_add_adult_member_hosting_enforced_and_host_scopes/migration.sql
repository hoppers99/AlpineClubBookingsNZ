...
--   2. Three NULLABLE, NO-DEFAULT boolean columns on "AdultMemberHostingPolicy"
...
--      pre-#2569 rule, so no club's behaviour moves at migration time and no
--      existing policy is silently broadened to any-member-at-the-lodge (#2569 §15).
--   3. A CHECK holding the three columns to all-NULL or all-set, so a
...
-- ---------------------------------------------------------------------------
--
-- Three separate boolean columns rather than one enum or a bitmask, because the
-- owner's decision is explicit that the scopes are INDEPENDENT CHECKBOXES
-- combined with OR, and any combination is legal. Three columns say that in the
-- schema; an enum of the seven legal combinations would say it only in
-- application code, and adding a fourth scope later would then be an enum
-- change rather than another additive column.
--
...
ALTER TABLE "AdultMemberHostingPolicy"
    ...
    ADD COLUMN "hostScopeAnyMemberAtLodge" BOOLEAN,
    ADD COLUMN "hostScopeNominatedHost" BOOLEAN;
...
-- nominated-host NULL" has no defensible reading — is the nominated scope off,
-- or inherited from the club while the other two are custom? Refusing the shape
...
-- only the API route.
--
-- Written as an equality of NULL-counts rather than three ORed clauses so it
-- stays readable, and so adding a fourth scope column is a one-line edit.
-- Every existing row has all three NULL, so the constraint validates against
-- the current contents without a single violation.
ALTER TABLE "AdultMemberHostingPolicy"
    ADD CONSTRAINT "AdultMemberHostingPolicy_host_scopes_all_or_none"
    CHECK (
        (
            ("hostScopeSameBooking" IS NULL)::int
            + ("hostScopeAnyMemberAtLodge" IS NULL)::int
            + ("hostScopeNominatedHost" IS NULL)::int
        ) IN (0, 3)
    );
...
-- won the compare-and-swap. Adding the three columns to the comparison makes a
...
--
-- Old-colour safety: a draining old colour's UPDATE does not name the three new
...
BEGIN
    ...
    IF ROW(
        ...
        NEW."hostScopeAnyMemberAtLodge",
        NEW."hostScopeNominatedHost"
        ...
        OLD."hostScopeAnyMemberAtLodge",
        OLD."hostScopeNominatedHost"
    ) THEN
