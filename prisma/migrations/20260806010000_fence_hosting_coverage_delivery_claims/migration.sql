...
--
-- Old clients ignore these columns. New clients treat a NULL re-evaluation token
-- as unclaimed and use the explicit expiry for crash recovery; notification
-- completion/release additionally names the opaque claimant token so a stale
-- sender cannot clear a successor's lease.
