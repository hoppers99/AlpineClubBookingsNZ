...
describe("roster-date lock source contract (#2586)", () => {
  ...
  it("pins direct whole-roster and row writers to the shared roster-date lock", () => {
    expect(source("src/lib/admin-roster-service.ts")).toMatch(/await lockRosterDate\(tx, date\)/)
    expect(source("src/app/api/lodge/roster/[date]/confirm/route.ts")).toMatch(/await lockRosterDate\(tx, date\)/)
