...
> = {
  ...
  subscription_unpaid_hard_block:
    "The club REFUSES this booking outright: it is a saved draft, the club's subscription-lockout policy is HARD_BLOCK, and the booking owner owes an unpaid season subscription. Confirming the draft returns 403 and no Booking Officer can except it — the only remedy is settling the subscription (or an administrator confirming on the member's behalf, which bypasses the gate). This is reported ONLY on a draft, because a draft's confirm is the door the club gates; it is not reported on an already-confirmed booking, where the same unpaid subscription blocks nothing about that booking. It also does not fire under NON_MEMBER_PRICING, where the unpaid member confirms and is repriced instead — see policy_paid_up_adult_member for that regime.",
...
BLOCKER CODES. blockerCodes is in PRIORITY order — report the first one as the primary problem and mention the rest as also true. Absent means nothing is blocking. Each code's exact meaning is in this tool's own description; use those words and do not paraphrase them. One limit matters here: subscription_unpaid_hard_block is raised only on a DRAFT, so its absence on a confirmed booking says nothing about the owner's subscription — member_eligibility_state answers that.
