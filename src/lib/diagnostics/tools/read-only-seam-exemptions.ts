...
export const READ_ONLY_SEAM_EXEMPTIONS: readonly DiagnosticsReadOnlySeamExemption[] =
  [
    ...
    },
    {
      ...
      symbol: "loadEffectiveModuleFlagsStrict",
      reason:
        "The readiness entry needs the club's module flags to say whether diagnostics is switched on, and it reads them on the application's own client outside the seam and TOLERATES a failure of that read rather than rejecting, because a readiness check that cannot answer when the database is unreachable has failed at the one moment it exists for; the seam would convert that same fault into a rejection. Since #2803 the tolerance is the readiness caller's own catch around the STRICT loader, not a loader that swallows errors and returns every flag false.",
      residual:
        "What this row used to cost is fixed and what it still costs is smaller but real, so both are stated. FIXED (#2803): a NARROW failure of just this one query no longer reads as a settled answer. readDiagnosticsModuleFlag catches around loadEffectiveModuleFlagsStrict and returns null, so the row reports module_enabled: null with blocker_codes: module_flags_unreadable, which an operator and the model can both tell apart from a club that genuinely switched diagnostics off; the model-facing catalogue says in as many words that it is not evidence the module is off. REMAINING: the read is still outside the READ ONLY fence, so only this code — not the server — makes it read-only, and a transient timeout on that one query now fails readiness CLOSED, which is the correct trade but means a momentary fault is reported as a blocker an operator has to investigate rather than as a state that clears itself.",
    },
    {
