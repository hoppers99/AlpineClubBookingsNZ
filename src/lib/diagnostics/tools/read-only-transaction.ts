 ...
 * takes no transaction client. Those are `READ_ONLY_SEAM_EXEMPTIONS` below: a
 * closed table with a reason per row, named by each entry's own `readOnlySeam`
 * declaration, refused at definition time if the id does not exist, and pinned by a
 * census test so a fifth exemption is a reviewed decision rather than an accident.
 */
 ...
 *
 * `module` and `symbol` name the thing that is exempt, not the entry that calls
 * it, because that is what a reviewer has to look at: three of the four rows are
 * SHARED helpers whose behaviour belongs to another feature, and re-homing the
 * entry would not change any of them.
 */
...
/**
 * THE CLOSED WORLD OF WHAT DOES NOT RUN INSIDE THE SEAM (#2786).
 *
 * Four rows, each a structural fact rather than a preference — and stating them is
 * the point. The claim "every `server_owned` entry reads through the seam" is not
 * satisfiable, and a contract that overstates itself is worse than one that names
 * its holes: it teaches the next author that the guarantee already covers the case
 * they are about to add.
 *
 * An entry declares the ids it relies on in its own `readOnlySeam` field.
 * `define.ts` refuses at definition time to register a `server_owned` entry that
 * names an id absent from this table, and refuses one that reads outside the seam
 * while naming no id at all — so this table, and not a reviewer's memory, is the
 * only sanctioned bypass. `__tests__/read-only-transaction.test.ts` pins the row
 * set exactly, so a fifth row is a decision somebody made in a diff.
 */
...
/** Every declared exemption id. The type an entry's declaration is checked against. */
